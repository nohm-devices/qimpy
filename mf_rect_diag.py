"""1st-order HLLC to steady state: confirm the current flows source->drain, the contacts
impose the reservoir density, and the density dipole is small only because the flow is deeply
subsonic (nearly incompressible).  Saves a density-deviation + current-streamline plot."""
...
fs._fluid_use_hllc = True; fs._fluid_first_order = True
...
n0 = float(fv._U[:, 0].mean())
# reservoir density targets
nres_s = float(fs.n_FD(torch.tensor(fs.E_F + 0.1), torch.tensor(fs.T_temp)))
nres_d = float(fs.n_FD(torch.tensor(fs.E_F - 0.1), torch.tensor(fs.T_temp)))
print(f"E_F={fs.E_F:.4f}  n0={n0:.5f}  reservoir n(+0.1)/n0={nres_s/n0:.4f}  n(-0.1)/n0={nres_d/n0:.4f}", flush=True)

prev = fv._U.clone(); tsim = 0.
for blk in range(160):
    for _ in range(50):
    ...
    prev = fv._U.clone()
print(f"stopped at block {blk}, rel={ch:.2e}", flush=True)

n = fv._U[:, 0]; Jx, Jy = fv._U[:, 1], fv._U[:, 2]
mu, Te, u = fs.recover_frame(fv._U)
# nearest cells to the ports
i_s = int(((cen[:, 0] - 10.) ** 2 + (cen[:, 1] - 55.) ** 2).argmin())
i_d = int(((cen[:, 0] - 10.) ** 2 + (cen[:, 1] - 5.) ** 2).argmin())
print(f"contact-cell density:  source n/n0={float(n[i_s])/n0:.4f}  drain n/n0={float(n[i_d])/n0:.4f}  "
      f"(reservoir targets {nres_s/n0:.4f} / {nres_d/n0:.4f})", flush=True)
...
print(f"current |J|: max={float(Jmag.max()):.4e} mean={float(Jmag.mean()):.4e}  drift Mach max={float(u.norm(dim=1).max())/float(fs.sound_speed(mu,Te).max()):.4f}", flush=True)
# net current leaving source region downward vs into drain (rough conservation check)
print(f"mean Jy in source half (y>30)={float(Jy[cen[:,1]>30].mean()):+.4e}  drain half (y<30)={float(Jy[cen[:,1]<30].mean()):+.4e} (both should be <0: net downward source->drain)", flush=True)

# ---- plot: density deviation + current streamlines ----
tri = Triangulation(Vv[:, 0], Vv[:, 1], Tn); nn = len(Vv); w = fv.geom.area.cpu().numpy()
nnp = n.cpu().numpy(); drho = nnp - nnp.mean()
def node(cv):
...
ax.streamplot(gxg, gyg, Ui, Vi, color="k", density=1.7,
              linewidth=0.3 + 2.4 * np.minimum(sp / (sp.max() + 1e-30), 1.0), arrowsize=0.8)
...
cb = fig.colorbar(tpc, ax=ax, shrink=.85, pad=.01); cb.set_label(r"$n-\langle n\rangle$")
ax.set_aspect("equal"); ax.set_xlim(X0, X1); ax.set_ylim(Y0, Y1); ax.set_xticks([]); ax.set_yticks([])
ax.set_title("qimpy fluid (HLLC, 1st-order) STEADY: density + current streamlines "
             f"(drift Mach {float(u.norm(dim=1).max())/float(fs.sound_speed(mu,Te).max()):.3f})", fontsize=10)
fig.tight_layout(); fig.savefig("/tmp/rect_1storder.png", dpi=120); print("wrote /tmp/rect_1storder.png", flush=True)
